...
export class VerifyResponseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _verifyResponseService: VerifyResponseService
  ) {}

  ngOnInit() {
    this.paramID = this._activatedRoute.snapshot.queryParamMap.get("id");
    console.log("VERIFYID", this.paramID);
    this._verifyResponseService.getVerifiedResponse(
      this.paramID,
      sessionStorage.getItem("cspace")
    );
    this._dataSubscriber = this._verifyResponseService
  ...
  onVerifyStatus(form: NgForm) {
    ...
    this._verifyResponseService.setVerificationResponse(
