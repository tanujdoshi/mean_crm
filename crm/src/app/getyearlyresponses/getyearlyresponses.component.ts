...
export class GetyearlyresponsesComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private getYearlyResponsesService: GetyearlyresponsesService,
  ...
  ngOnInit() {
    // this.yearlySubscriber = this.getYearlyResponsesService
    //   .getYearlysubs()
    //   .subscribe((res: any) => {
  ...
  onSubmit(form: NgForm) {
    if (Number.isInteger(+form.value.year) && form.value.year.length === 4) {
      ...
      this.getYearlyResponsesService.getYearlyResponses(
        form.value.year,
      ...
      this.yearlySubscriber = this.getYearlyResponsesService
        ...
        .subscribe((res: any) => {
          ...
          for (const key of this.keys) {
            // console.log(this.keys[i])
            this.columnDefs.push({ header: key, field: key });
          }
          const objToRemove = { header: 'verifystatus', field: 'verifystatus' };
          console.log(...this.columnDefs);
